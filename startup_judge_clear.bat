...
call :resolve_npm
set "BCI_MATCH_START_MODE=clear"
...
if errorlevel 1 (
  echo [startup_judge_clear] startup failed with exit code %errorlevel%.
  pause
  exit /b %errorlevel%
)
exit /b 0
...
if not defined BCI_PYTHON_EXE (
  echo [startup_judge_clear] Python not found.
  pause
  exit /b 1
)
exit /b 0
...
if not defined BCI_JAVA_EXE (
  echo [startup_judge_clear] Java not found.
  pause
  exit /b 1
)
exit /b 0
...
if not defined BCI_NPM_EXE (
  echo [startup_judge_clear] npm not found, judge-dashboard will not be auto-started.
)
exit /b 0
