    ...
    def __virtual_receiver_custom_control_message_to_model(
        ...
        return VirtualReceiverCustomControlModel(
            ...
            CalibrationTrialCountControlPackageModel(
                team_id=(
                    virtual_receiver_custom_control_message.calibrationTrialCountControlMessage.teamId
                ),
                calibration_trial_count_per_class=(
                    virtual_receiver_custom_control_message.calibrationTrialCountControlMessage
                    .calibrationTrialCountPerClass
                )
            )
            if virtual_receiver_custom_control_message.WhichOneof(
                'package') == "calibrationTrialCountControlMessage" else
            None
        )
        ...
        if isinstance(package, CalibrationTrialCountControlPackageModel):
            return VirtualReceiverCustomControlMessage_pb2(
                calibrationTrialCountControlMessage=CalibrationTrialCountControlMessage_pb2(
                    teamId=virtual_receiver_custom_control_model.package.team_id or "",
                    calibrationTrialCountPerClass=(
                        virtual_receiver_custom_control_model.package.calibration_trial_count_per_class
                    )
                )
            )
