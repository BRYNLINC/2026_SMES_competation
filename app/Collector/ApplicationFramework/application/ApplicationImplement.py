...
class ApplicationImplement(ApplicationInterface):
    ...
    def __init__(self):
        super().__init__()
        # 从接口继承 self._component_framework
    ...
    async def run(self) -> None:
        ...
        for source_name in source_dict:
            ...
            await self._component_framework.subscribe_message(source_name, ReceiveMessageCallback())
        ...
        await self._component_framework.send_message("test_message_key", b"test_message")
        ...
        await self._component_framework.add_listener_on_update_component_info(UpdateConfigCallback())
        # 测试组件配置拉取
        print(self._component_framework.get_component_model())
        # 测试组件配置更新
        await self._component_framework.update_component_info({"test_key": "test_value"})
        # 测试全局配置订阅
        await self._component_framework.add_listener_on_update_global_config(UpdateConfigCallback())
        # 测试全局配置拉取
        print(self._component_framework.get_global_config())
        # 测试全局配置更新
        await self._component_framework.update_global_config({"test_key": "test_value"})
