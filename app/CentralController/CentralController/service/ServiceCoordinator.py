...
class ServiceCoordinator(ServiceCoordinatorInterface):
    ...
    它负责管理中控的两份核心信息：
    1. 静态配置：来自 CentralControllerConfig.yml；
    2. 运行时注册表：当前有哪些组件已经注册。

    组件注册、消息绑定时，最终都要经过这里来补充默认配置或决定最终 topic。
    """
    ...
    async def initial(self):
        ...
        # 这个配置文件决定了：
        # 1. 组件属于哪个 group；
        # 2. 默认 message_key -> topic 映射；
        # 3. 某些组件默认的 component_info。
    ...
    async def startup(self) -> None:
        # 如果中控启动时已经存在已注册组件，这里会把它们重新装入运行时注册表，
        # 并同步套用静态配置。
    ...
    async def on_register_component(self, component_model: ComponentModel) -> ComponentModel:
        # 组件注册时，先写入运行时注册表；
        # 如果该组件在静态配置中存在，再把静态 component_info / topic 映射补进去。
    ...
    async def on_bind_message(self, message_binding_model: MessageBindingModel) -> MessageBindingModel:
        # 组件申请绑定 topic 时，优先采用中控静态配置里的 topic；
        # 如果静态配置没有写，才使用组件自己传来的 topic。
