...
from ProcessHub.api.model.ProcessHubControlModel import ProcessHubControlModel, ApplicationExitControlModel
...
class ProcessorConnector(ProcessorConnectorInterface):
    ...
    async def application_exit(self, component_id: str):
        ...
        send_model = ProcessHubControlModel(
            package=ApplicationExitControlModel()
        )
        proto = ProcessHubControlMessageConverter.model_to_protobuf(send_model)
        # 这个退出命令最终由：
        # app/ProcessHub/ProcessHub/control/CommandController.py
        # 里的 ReceiveCommandControlMessageOperator.receive_message() 接收。
