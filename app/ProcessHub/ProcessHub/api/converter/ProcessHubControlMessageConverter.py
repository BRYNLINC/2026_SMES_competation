...
class ProcessHubControlMessageConverter:
    ...
    def initial(cls):
        cls.__package_name_for_convert_func_dict = {
            ...
            ProcessHubControlMessage_pb2: cls.__process_hub_control_message_to_model,
        }
        cls.__model_class_for_convert_func_dict = {
            ApplicationExitControlModel: cls.__application_exit_control_model_to_message_pb,
            ProcessHubControlModel: cls.__process_hub_control_model_to_message_pb,
        }
    ...
    @classmethod
    def __process_hub_control_message_to_model(
            cls, process_hub_control_message: ProcessHubControlMessage_pb2) -> ProcessHubControlModel:
        package_name = process_hub_control_message.WhichOneof('package')
        return ProcessHubControlModel(
            package=cls.__application_exit_control_message_to_model(
                process_hub_control_message.applicationExitControlMessage)
    ...
    @classmethod
    def __process_hub_control_model_to_message_pb(cls, process_hub_control_model: ProcessHubControlModel) \
            -> ProcessHubControlMessage_pb2:
        package = process_hub_control_model.package
        if isinstance(package, ApplicationExitControlModel):
            return ProcessHubControlMessage_pb2(
                applicationExitControlMessage=cls.__application_exit_control_model_to_message_pb(
                    process_hub_control_model.package)
            )
