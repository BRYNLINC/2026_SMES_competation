...
class MessageManagerInterfaceImpl(MessageManagerInterface):
    ...
    async def subscribe_topic(self, callback, message_key):
        ...
        subscribe_task = asyncio.create_task(self.__message_forwarder.subscribe_topic(operator, message_key))
        subscribe_task.add_done_callback(
            lambda task: self.__log_subscription_task_exception(task, message_key)
        )
    ...
    def __log_subscription_task_exception(self, task: asyncio.Task, message_key: str) -> None:
        if task.cancelled():
            return
        ...
        self.__logger.exception(
            "subscribe_topic 后台任务异常: message_key=%s",
            message_key,
            exc_info=exception,
        )
