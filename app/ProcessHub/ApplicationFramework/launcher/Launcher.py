...
class Launcher:
    # CentralController Python 侧的 componentframework 服务(127.0.0.1:9003)
    # 在 startup.bat 链路里有明显启动时延。Coordinator / ProcessHub 过早启动时，
    # 常见现象就是 grpc UNAVAILABLE: connection refused。
    # 这里统一给 ProcessHub 系组件做有限次重试，避免依赖人工卡 timeout。
    __FRAMEWORK_STARTUP_MAX_RETRY = 20
    __FRAMEWORK_STARTUP_RETRY_INTERVAL_SECONDS = 1.0
    ...
    async def initial(self):
        ...
        ensure_logging_targets(logging_config, base_dir=Path(self.__root_path))
        # 应用配置到logging模块
        logging.config.dictConfig(logging_config)
        self.__logger = logging.getLogger('componentLogger')
        # 创建component_framework并封装context
        self.__component_framework = ComponentFrameworkImplement()
        # 写入配置信息
        ...
        # 初始化application
        # 获取当前脚本所在的目录
        launcher_config_file_path = os.environ.get(
            'LAUNCHER_CONFIG_PATH',
            os.path.join(self.__root_path, 'config', 'LauncherConfig.yml')
        )
        ...
        self.__application.set_component_framework(self.__component_framework)
        # 初始化应用实例
        await self.__application.initial()

    async def startup(self):
        # 启动组件框架
        await self.__startup_component_framework_with_retry()
        ...
        try:
            await self.__register_component_with_retry(component_model)
    ...
    async def __startup_component_framework_with_retry(self) -> None:
        last_exception = None
        for attempt in range(1, self.__FRAMEWORK_STARTUP_MAX_RETRY + 1):
            try:
                self.__logger.info(
                    "组件框架启动尝试: attempt=%s/%s daemon=%s:%s",
                    attempt,
                    self.__FRAMEWORK_STARTUP_MAX_RETRY,
                    self.__daemon_address,
                    self.__daemon_port,
                )
                await self.__component_framework.startup()
                self.__logger.info("组件框架启动成功")
                return
            except Exception as exc:
                last_exception = exc
                self.__logger.warning(
                    "组件框架启动失败，等待重试: attempt=%s/%s error=%s",
                    attempt,
                    self.__FRAMEWORK_STARTUP_MAX_RETRY,
                    exc,
                )
                if attempt < self.__FRAMEWORK_STARTUP_MAX_RETRY:
                    await asyncio.sleep(self.__FRAMEWORK_STARTUP_RETRY_INTERVAL_SECONDS)
        raise last_exception

    async def __register_component_with_retry(self, component_model) -> None:
        last_exception = None
        for attempt in range(1, self.__FRAMEWORK_STARTUP_MAX_RETRY + 1):
            try:
                self.__logger.info(
                    "组件注册尝试: attempt=%s/%s component_id=%s component_type=%s",
                    attempt,
                    self.__FRAMEWORK_STARTUP_MAX_RETRY,
                ...
                self.__logger.info(
                    "组件注册成功: component_id=%s component_type=%s",
                    component_model.component_id,
                    component_model.component_type,
                )
                return
                ...
                last_exception = exc
                self.__logger.warning(
                    "组件注册失败，等待重试: attempt=%s/%s component_id=%s error=%s",
                    attempt,
                    self.__FRAMEWORK_STARTUP_MAX_RETRY,
                    component_model.component_id,
                    exc,
